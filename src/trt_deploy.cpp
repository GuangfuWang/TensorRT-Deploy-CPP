...
void TrtDeploy::Init(const std::string &model_file)
{
	...
	if (!m_runtime) {
		m_runtime = createSharedRef<nvinfer1::IRuntime>(nvinfer1::createInferRuntime(*m_logger));
		initLibNvInferPlugins(m_logger.get(), "");
		auto engine_ptr = m_runtime->deserializeCudaEngine((void *)&buf[0], mSize);
		m_engine            = createSharedRef<nvinfer1::ICudaEngine>(engine_ptr);
		m_execution_context = createSharedRef<nvinfer1::IExecutionContext>(m_engine->createExecutionContext());
	}

	auto in_dims = m_engine->getTensorShape(Config::INPUT_NAME.c_str());
	///note the target size should match the model input.
	assert(Config::TARGET_SIZE[1] == in_dims.d[in_dims.nbDims - 1] &&
		Config::TARGET_SIZE[0] == in_dims.d[in_dims.nbDims - 2]);
	int      in_size = 1;
	for (int i       = 0; i < in_dims.nbDims; ++i) {
		in_size *= in_dims.d[i];
	}
	m_input_state.resize(in_size);
	m_output_state.resize(Config::OUTPUT_NAMES.size());
	m_device_ptr.resize(1 + Config::OUTPUT_NAMES.size());
	for (int i = 0; i < Config::OUTPUT_NAMES.size(); ++i) {
	...
	m_execution_context->setTensorAddress(Config::INPUT_NAME.c_str(),
										  m_device_ptr[0]);
	for (int i = 0; i < Config::OUTPUT_NAMES.size(); ++i) {
...
void TrtDeploy::InferResults(SharedRef<ImageBlob> &data, SharedRef<TrtResults> &res)
{
	res->Clear();
	cudaError_t state =
					cudaMemcpyAsync(m_device_ptr[0], &data->im_data_[0],
									m_input_state.size() * sizeof(float), cudaMemcpyHostToDevice, m_stream);
	if (state) {
	...
	for (int  i   = 0; i < Config::OUTPUT_NAMES.size(); ++i) {
		state = cudaMemcpyAsync(&m_output_state[i][0], m_device_ptr[i + 1],
								m_output_state[i].size() * sizeof(float),
